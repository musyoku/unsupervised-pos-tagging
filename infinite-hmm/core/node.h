...
private:
	// 客をテーブルに追加
	bool add_customer_to_table(int token_id, int table_k, double parent_Pw, vector<double> &theta_m, int &added_to_table_k){
	...
	bool add_customer_to_new_table(int token_id, double parent_Pw, vector<double> &theta_m, int &added_to_table_k){
		if(_arrangement.find(token_id) == _arrangement.end()){
			vector<int> tables = {1};
...
public:
	...
	bool add_customer(int token_id, double g0, double alpha, double beta, double gamma){
		double parent_Pw = g0;
		if(_parent){
			parent_Pw = _parent->compute_Pw(token_id, g0, d_m, theta_m);
		}
		if(_arrangement.find(token_id) == _arrangement.end()){
			add_customer_to_new_table(token_id, parent_Pw, alpha, beta, gamma);
			return true;
		}
		vector<int> &num_customers_at_table = _arrangement[token_id];
		double sum_prob = 0;
		for(int k = 0;k < num_customers_at_table.size();k++){
			sum_prob += std::max(0.0, num_customers_at_table[k]);
		}
		double t_u = _num_tables;
		sum_prob += theta_u * parent_Pw;
	...
	bool remove_customer(int token_id, bool update_n, int &removed_from_table_k){
		...
		vector<int> &num_customers_at_table = _arrangement[token_id];
		double sum_prob = std::accumulate(num_customers_at_table.begin(), num_customers_at_table.end(), 0);		
		double normalizer = 1.0 / sum_prob;
		double r = Sampler::uniform(0, 1);
		double sum_normalized_probs = 0.0;
		for(int k = 0;k < num_customers_at_table.size();k++){
